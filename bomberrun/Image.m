...
- (Image*)subImageInRect:(CGRect)aRect {
    ...
    Image *subImage = [[Image alloc] initWithImageNamed:imageFileName filter:GL_LINEAR subTexture:aRect];
...
- (void)setImageSizeToRender:(CGSize)aImageSize {
	...
	if (aImageSize.width < 0 || aImageSize.width > 100 || aImageSize.height < 0 || aImageSize.height > 100) {
		SLQLOG(@"ERROR - Image: Illegal  provided to setImageSizeToRender 'width=%lf, height=%lf'", aImageSize.width, aImageSize.height);
		return;
	}
